...
public class Main extends Application {
    ...
    private static final String[] FOODS_IMAGE = new String[]{"/img/ic_pomegranate.png"};
    ...
    private void run(GraphicsContext gc) {
        if (gameOver) {
            ...
            ((Group) gc.getCanvas().getParent()).getChildren().add(vbox);
            return;
        }
        drawBackground(gc);
        drawFood(gc);
    ...
    private void resetGame() {
        ...
        Group root = (Group) gc.getCanvas().getParent();
        VBox vbox = (VBox) root.getChildren().get(root.getChildren().size() - 1);
